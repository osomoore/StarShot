...
Deck definitions are human-editable TOML files inside deck-set folders. Current v2 gameplay, balancing, AI battle analysis, and new tests should use the admin-selected active deck set rather than a bundled deck chosen by path or scan order. Simple card behavior is written as controlled-English `side_a_*` / `side_b_*` text, with generated copy ids from `name` and `copies = N`. The loader validates the deck set at load time and exposes the active catalog through existing deck helpers. Use `python scripts\server_control.py start --deck-set path\to\deck_set` or `STARSHOT_DECK_SET` to swap deck sets for legacy/non-v2 tools. New games store `deck_set_id`, and gameplay operations reject saved games whose deck set differs from the active server catalog. See `docs/context/deck_data.md`.
